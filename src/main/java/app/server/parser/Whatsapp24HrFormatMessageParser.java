...
public class Whatsapp24HrFormatMessageParser implements IWhatsappMessageParser {

  private static final String MAIN_REGEX = "^(\\d+\\/\\d+\\/\\d+),.(\\d+:\\d+).-.([^:]*):(.*)";
  private final DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm", Locale.US);
  ...
  public List<WhatsappMessage> parseFile(File file) throws IOException, UnsupportedExportFileException {
    ...
    try (Scanner scanner = new Scanner(file)) {
      while (scanner.hasNext()) {
        ...
        String sender = parsedGroups.get(3);
        String message = parsedGroups.get(4);
